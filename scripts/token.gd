...
var movement_used : float = 0.0
var allow_count_move: bool = true
...
func _ready():
	pos = grid.local_to_map(position)
...
func move(delta):
	if path.size() > 0:
		if position.distance_to(grid.map_to_local(path[0])) < 5:
			position = grid.map_to_local(path[0])
			pos = path[0]
			path.pop_front()
		else:
			position += (grid.map_to_local(path[0]) - position).normalized() * data.speed * delta
...
func limit_path(_path: Array[Vector2i]):
	...
	for i in _path.size()-1:
		...
		if p.x + p.y == 1:
			...
			movement_used += 1.5
		if floor(movement_used) <= data.move_speed:
...
func _input(event):
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed == true:
			...
			var clicked = grid.local_to_map(get_global_mouse_position())
			var total_path = pf.generate_path(pos, clicked)
...
#				print(Vector2(total_path[1]).normalized().dot(Vector2(_next).normalized()))
			...
			for point in path:
				point = (Vector2(point) * 128 + Vector2(64,64))
